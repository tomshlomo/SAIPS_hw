function [I_ins, I_ref_shift] = align_fft(I_ins, I_ref, plot_flag, trim)
arguments
    I_ins (:,:) double
    I_ref (:,:) double
    plot_flag = 1
...
N_corr = 2^ceil(log2(N_ins + N_ref - 1));
M_corr = 2^ceil(log2(M_ins + M_ref - 1));
C = fft2(I_ins_med, N_corr, M_corr) .* conj(fft2(I_ref_med, N_corr, M_corr));
H = abs(fft2(fspecial('sobel'), N_corr, M_corr)).^2;
cx = ifft2( C .* H);
cy = ifft2( C .* H');
% 
% 
